...
erpnext.PointOfSale.Payment = class {
	...
	prepare_dom() {
		this.wrapper.append(
			`<section class="payment-container">
				...
				<div class="payment-actions">
					...
					<div class="send-to-kitchen-btn">${__("Send to Kitchen")}</div>
				</div>
			</section>`
		);
		...
		this.$component.find(".payment-actions").css({
			"display": "flex",
			"gap": "10px",
			"width": "100%",
		...
		this.$component.find(".send-to-kitchen-btn").css({
	...
	bind_events() {
		...
		this.$component.on("click", ".send-to-kitchen-btn", () => {
			const frm = this.events.get_frm();
			frappe.utils.print(
				frm.doc.doctype,
				frm.doc.name,
